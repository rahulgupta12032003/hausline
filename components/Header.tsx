    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <nav className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
        <div className="flex items-center justify-between">
          <Link href="/" className="text-xl sm:text-2xl font-bold font-pacifico text-amber-600 dark:text-amber-400 truncate">
            Hausline Interior
          </Link>
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            >
              <i className={`ri-${theme === 'light' ? 'sun' : 'moon'}-line text-lg sm:text-xl text-gray-700 dark:text-gray-300 group-hover:text-amber-600 group-hover:rotate-180 transition-all duration-500`}></i>
            </button>
            ...
            >
              <i className={`ri-${isMenuOpen ? 'close' : 'menu'}-line text-lg sm:text-xl`}></i>
            </button>
          </div>
        </div>

        {isMenuOpen && (
          <div className="md:hidden mt-4 py-4 border-t border-gray-200 dark:border-gray-700 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md rounded-lg -mx-4 px-4">
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
